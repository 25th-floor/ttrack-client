...
import {
    ...
    type ApiUserType,
} from '@data/Resources/ResourcesTypes';
...
export type ProcessedPeriodType = {
    type: ApiPeriodTypeType,
...
export function assocPeriodsWithTypes(types: Array<ApiPeriodTypeType>, days: Array<ApiDayType>): Array<ApiDayType> {
...
export type ProcessedDayType = ApiDayType & {
    weekNr: WeekNrType,
    date: Moment,
...
function processDay(day: ApiDayType): ProcessedDayType {
...
export function createWeeks(days: Array<ApiDayType>, carryTime: DurationType): Array<ProcessedWeekType> {
...
export function validatePeriod(period: ProcessedPeriodType): boolean {
    ...
    const typeConfig = R.pathOr({}, ['type', 'pty_config', 'types'])(period);

    // if there is no duration needed, don't ask for one (comment type for example)
    if (period.duration === NONE && !R.all((t => t))(R.values(typeConfig))) return true;
