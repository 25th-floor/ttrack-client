...
import type { DayType, DurationType, PeriodType, PeriodTypeType, UserType } from '@data/Resources/ResourcesTypes';
 ...
 * @returns boolean
 */
function isValidTimeString(time: string, strict: boolean = true, allowNegativeValue: boolean = false): boolean {
...
function reduceMomentArray(moments: Array<Moment>, user: UserType): Array<Moment> {
 ...
 * @returns {boolean}
 */
export function getNearestDateWithinEmployment(date: Moment, user: UserType): Moment | false {
    let startDate = user.usr_employment_start;
    let endDate = user.usr_employment_stop;
    // give one month more time before and after
    if (startDate) {
 ...
 * @returns {boolean}
 */
export function isDateInEmploymentInterval(date: Moment, user: UserType): boolean {
...
function durationOfBreak(period: PeriodType): Duration {
    if (!period) return 0;
    return moment.duration(period.per_break || 0);
}

function durationOfWork(period: PeriodType): Duration {
...
function durationOfBalance(period: PeriodType): Duration {
...
export function getFirstAndLastDayOfMonth(month: Moment): BoundaryType {
    const ret = {};
 ...
 */
export function getYearsForUser(user: UserType, _today: Moment, limit: number = 6): Array<Moment> {
    let today2 = _today;
 ...
 */
export function getMonthsForUser(user: UserType, activeMonth: Moment): Array<Moment> {
...
function getDurationType(period: PeriodType, targetTime: DurationType, type: PeriodTypeType): DurationVariantType {
...
export type AssocPeriodType = PeriodType & {
    type: PeriodTypeType,
    duration: DurationVariantType,
};

function assocPeriodWithType(typeMap: Object, targetTime: DurationType, period: PeriodType): AssocPeriodType {
    const type = typeMap[period.per_pty_id]; // PeriodTypeType
...
export type AssocDayType = DayType & {
    periods: Array<AssocPeriodType>,
};
export function assocPeriodsWithTypes(types: Array<PeriodTypeType>, days: Array<DayType>): Array<AssocDayType> {
...
function periodUnfinished(period: PeriodType): boolean {
...
export function createWeeks(days: DayType, carryTime: DurationType): Array<ProcessedWeekType> {
 ...
 * @returns {*}
 */
export function calculateDuration(period: AssocPeriodType, fullDay: Duration): DurationType {
...
function getBreakTime(period: AssocPeriodType): number {
    return moment.duration(period.per_break)
        .as('minutes');
}

function getWorkedTime(period: AssocPeriodType): number {
 ...
 * @returns {boolean}
 */
function isBreakLengthValid(period: AssocPeriodType): boolean {
 ...
 * @returns {boolean}
 */
export function isOverlapping(periods: Array<AssocPeriodType>): boolean {
    if (periods.length <= 1) {
        return false;
    }
...
export function validatePeriod(period: AssocPeriodType): boolean {
 ...
 * @returns {Array}
 */
export function getAllErrors(period: AssocPeriodType): Array<string> {
 ...
 * @returns {*|boolean}
 */
export function validatePeriods(periods: Array<AssocPeriodType>): boolean {
